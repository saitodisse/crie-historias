...
export async function importDataForUser(userId: number, data: ImportData) {
    ...
    await db.transaction(async (tx) => {
        ...
        if (userScriptIds.length > 0) {
            await tx.delete(scriptPrompts).where(inArray(scriptPrompts.scriptId, userScriptIds));
        }

        // scripts -> depends on projects
        if (userProjectIds.length > 0) {
            await tx.delete(scripts).where(inArray(scripts.projectId, userProjectIds));
        }

        // project_characters -> depends on projects (and characters)
        if (userProjectIds.length > 0) {
            await tx.delete(projectCharacters).where(inArray(projectCharacters.projectId, userProjectIds));
        }
        ...
        await tx.delete(creativeProfiles).where(eq(creativeProfiles.userId, userId));
